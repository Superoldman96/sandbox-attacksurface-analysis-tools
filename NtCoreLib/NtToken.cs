...
using NtCoreLib.Win32.AppModel;
using System;
...
public sealed class NtToken : NtObjectWithDuplicateAndInfo<NtToken, TokenAccessRights, TokenInformationClass, TokenInformationClass>
{
        ...
        AttributeFlags attributes, SecurityDescriptor security_descriptor, bool throw_on_error)
    {
        ...
        using ObjectAttributes obja = new(null, attributes, SafeKernelObjectHandle.Null, sqos, security_descriptor);
    ...
    public NtToken Filter(FilterTokenFlags flags, IEnumerable<Sid> sids_to_disable, IEnumerable<Luid> privileges_to_delete, IEnumerable<Sid> restricted_sids)
    {
        ...
        try
        {
            ...
            {
                TokenPrivilegesBuilder builder = new();
    ...
    public void SetDefaultDacl(Acl dacl)
    {
        ...
        TokenDefaultDacl default_dacl = new()
        {
    ...
    public NtResult<ClaimSecurityAttribute[]> GetSecurityAttributes(SecurityAttributeType type, bool throw_on_error)
    {
        ...
        List<ClaimSecurityAttribute> attributes = new();
    ...
    {
        Luid priv_value = new((uint)privilege, 0);
    ...
    {
        OptionalInt64 attributes_present_obj = new(0);
        PsPkgClaim pkg_claim = new();
    ...
    {
        var info_class = group_type switch
        {
            QueryGroupType.Default => TokenInformationClass.TokenGroups,
            QueryGroupType.Restricted => TokenInformationClass.TokenRestrictedSids,
            QueryGroupType.Capability => TokenInformationClass.TokenCapabilities,
            QueryGroupType.Device => TokenInformationClass.TokenDeviceGroups,
            QueryGroupType.RestrictedDevice => TokenInformationClass.TokenRestrictedDeviceGroups,
            _ => throw new ArgumentException("Invalid group type", "group_type"),
        };
    ...
    public override NtResult<object> QueryObject(TokenInformationClass info_class, bool throw_on_error)
    {
        ...
        {
            TokenInformationClass.TokenStatistics => (NtResult<object>)Query<TokenStatistics>(info_class, default, throw_on_error),
    ...
    public string PackageFullName
    {
        get
        {
            ...
            OptionalLength package_length = new(package_name.Length);
    ...
    public string AppId
    {
        get
        {
            ...
            OptionalLength app_id_length = new(app_id.Length);
                ...
                bool throw_on_error)
    {
        ...
        TokenPrimaryGroup primary_group_struct = new()
        {
    ...
    private NtResult<NtToken> PseudoToHandle(TokenAccessRights desired_access, bool throw_on_error)
    {
        ...
        return Handle.DangerousGetHandle().ToInt32() switch
        {
            -4 => OpenProcessToken(NtProcess.Current, desired_access, throw_on_error),
            -5 => OpenThreadToken(NtThread.Current, true, false, desired_access, throw_on_error),
            -6 => OpenEffectiveToken(NtThread.Current, true, false, desired_access, throw_on_error),
            _ => throw new ArgumentException("Invalid pseudo handle."),
        };
    }
    ...
    private NtStatus SetSecurityAttributes(ClaimSecurityAttributeBuilder[] attributes, TokenSecurityAttributeOperation[] operations, bool throw_on_error)
    {
        ...
        TokenSecurityAttributesAndOperationInformation info = new()
        {
    ...
    private static bool GetSecurityAttributeNative(SecurityAttributeType type)
    {
        return type switch
        {
            SecurityAttributeType.Local or SecurityAttributeType.Singleton => true,
            _ => false,
        };
    }
    ...
    private static TokenInformationClass GetSecurityAttributeClass(SecurityAttributeType type)
    {
        return type switch
        {
            SecurityAttributeType.Local => TokenInformationClass.TokenSecurityAttributes,
            SecurityAttributeType.User => TokenInformationClass.TokenUserClaimAttributes,
            SecurityAttributeType.RestrictedUser => TokenInformationClass.TokenRestrictedUserClaimAttributes,
            SecurityAttributeType.Device => TokenInformationClass.TokenDeviceClaimAttributes,
            SecurityAttributeType.RestrictedDevice => TokenInformationClass.TokenRestrictedDeviceClaimAttributes,
            SecurityAttributeType.Singleton => TokenInformationClass.TokenSingletonAttributes,
            _ => throw new ArgumentException("Invalid attributes type."),
        };
    }
